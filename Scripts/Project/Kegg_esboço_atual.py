...
def criar_caixas_por_amostra(taxa_expressions : dict, taxa_colors : dict) -> str:
    """
    Gera HTML para exibir as expressões diferenciais de amostras em forma de caixas verticais coloridas.
    ...
    -------
        str
            HTML para incorporar ao corpo principal da página.
    """
    if not isinstance(taxa_colors, dict):
        raise ValueError("taxa_colors deve ser um dicionário de cores")

    html_content = '<div style="display: flex; justify-content: center; width: 100%;">'
    for amostra in ['MP1', 'MP2', 'MP3', 'MP4']:
        html_content += f'<div style="margin: 10px; padding: 10px; border: 1px solid black;">'
        ...
        if total_expression == 0:  
            ...
            for taxa, values in taxa_expressions.items():
                ...
                html_content += f'<div style="background-color: {color}; width: 50px; height: {height}; margin-bottom: 2px;">{values[amostra]}</div>'
        html_content += '</div>'
    html_content += '</div>'
    return html_content
...
def criar_pagina_detalhe(id : str, link : str, ec_ko : str, KO : list[str], Taxon : list[str], colors_dict : dict[str, list[str]], taxa_expressions : dict, taxa_colors : dict):
    ...
    <body>
        <div class="grid-container">
            ...
            <div class="grid-item" id="sample_boxes">
                {sample_boxes}  <!-- Adicionando as caixas das amostras -->
            </div>
